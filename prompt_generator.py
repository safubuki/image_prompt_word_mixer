...
class PromptGeneratorApp:
    ...
    def create_widgets(self):
        ...
        self.basic_combobox = ttk.Combobox(basic_frame, values=[prompt["name"] for prompt in self.basic_prompts])
        self.basic_combobox.grid(row=0, column=0, padx=5, pady=5)
        self.basic_combobox.bind("<<ComboboxSelected>>", self.on_basic_select)

        self.basic_text = tk.Text(basic_frame, height=10, width=50)
    ...
    def on_basic_select(self, _):
        ...
        if selection >= 0:
            ...
            self.update_variable_entries(prompt["text"])

    def update_variable_entries(self, text):
