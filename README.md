...
4. **JSON ファイルの作成方法**  
   ...
   - **基本プロンプト (basic_prompts.json)**
     ...
     基本プロンプトは生成する画像のベースとなるテンプレート情報を含み、テンプレートと変数の初期値が定義されています。ファイル例:
   ...
   - **追加プロンプト (element_prompts.json)**
     ...
     追加プロンプトは基本プロンプトに追加するオプション文章を管理します。今回、一部のキー名が変更され、従来の `prompts` が `prompt_lists` に、各プロンプトの名称が `name` から `title` に、そして `text` が `prompt` に変更されています。ファイル例:
